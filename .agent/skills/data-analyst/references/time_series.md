...
- **If p-value < 0.05**: Reject null â†’ Series is stationary
- **If p-value > 0.05**: Fail to reject â†’ Series is non-stationary

#### KPSS Test
- **Null hypothesis**: Series is stationary
- **If p-value < 0.05**: Reject null â†’ Series is non-stationary
- **If p-value > 0.05**: Fail to reject â†’ Series is stationary
...
| Multiplicative | Y = Trend Ã— Seasonal Ã— Residual | Seasonal variation scales with level |
...
2. For each subsequent day:
   ...
   - If rolling_mean >= baseline Ã— threshold:
...
#### Momentum
```
momentum_7d = (last_7d_mean - prev_7d_mean) / prev_7d_mean Ã— 100
```

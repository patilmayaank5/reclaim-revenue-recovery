...
- "Is this keyword trending?" â†’ growth_ratio > threshold
- "Is this user high-value?" â†’ lifetime_value > threshold
- "Is this metric anomalous?" â†’ value > upper_threshold OR value < lower_threshold
...
|-----------|----------------|-------------------|
| Mean Â± 1Ïƒ | Slightly unusual | 32% |
| Mean Â± 2Ïƒ | Unusual | 5% |
| Mean Â± 3Ïƒ | Highly unusual | 0.3% |

**Formula**: `threshold = mean Â± (n Ã— std_dev)`
...
```
threshold_traffic = cost / (conversion_rate Ã— avg_order_value)
```
...
| Growth | Stability | Recency | â†’ Category |
|--------|-----------|---------|------------|
| High (â‰¥2x) | Any | Recent (â‰¤14d) | RISING |
...
1. Vary threshold Â±10%, Â±20%, Â±50%

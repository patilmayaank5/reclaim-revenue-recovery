...
1. **State hypotheses**:
   - Null (Hâ‚€): No effect, no difference
   - Alternative (Hâ‚): There is an effect or difference

2. **Choose significance level (Î±)**: Usually 0.05

3. **Calculate test statistic**: Depends on the test

4. **Get p-value**: Probability of seeing this result if Hâ‚€ is true

5. **Make decision**:
   - p < Î±: Reject Hâ‚€ (evidence for effect)
   - p â‰¥ Î±: Fail to reject Hâ‚€ (insufficient evidence)

### Interpreting P-values

| P-value | Evidence Against Hâ‚€ |
|---------|---------------------|
...
- Sample size â‰¥ 50: Visual inspection (histogram, Q-Q plot)
...
| Scenario | Method |
|----------|--------|
| 2x2 table, expected counts â‰¥ 5 | Chi-square test |
...
#### Normality
- **Test**: Shapiro-Wilk (n < 50), Kolmogorov-Smirnov (n â‰¥ 50)
...
- If CI excludes 0: Effect is significant at Î± = 0.05
...
| Significance level | Use Î± = 0.10 instead of 0.05 |
...
| Chi-square | Expected count â‰¥ 5 per cell |
| ANOVA | 20+ per group |
...
- 1 test at Î± = 0.05: 5% false positive chance
- 20 tests at Î± = 0.05: 64% chance of at least one false positive!

### Corrections

#### Bonferroni
```
adjusted_Î± = Î± / number_of_tests
```
- Most conservative
...
2. **Ignoring effect size**: Statistical â‰  practical significance

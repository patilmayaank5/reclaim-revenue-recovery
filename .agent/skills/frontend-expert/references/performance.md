...
export const DataDisplay: React.FC<{ items: Item[], searchTerm: string }> = ({
    ...
    // âŒ AVOID - Runs on every render
...
```typescript
// âŒ AVOID - Creates new function on every render
...
export const MyForm: React.FC = () => {
    ...
    // âŒ AVOID - Watches all fields, re-renders on any change
...
// âŒ AVOID - Index as key (unstable if list changes)
...
```typescript
// âŒ AVOID - Component recreated on every render
export const Parent: React.FC = () => {
...
```typescript
// âŒ AVOID - Import heavy libraries at top level

...
```typescript
// âŒ NEVER DO THIS - Early return with loading spinner
const Component = () => {
...
```typescript
// âŒ NEVER - Early return
if (isLoading) {
    return <CircularProgress />;
}

// âŒ NEVER - Conditional rendering
{isLoading ? <Spinner /> : <Content />}

// âŒ NEVER - Layout changes
if (isLoading) {
    return (
        <Box sx={{ height: 100 }}>
            <Spinner />
...
- âŒ **NEVER**: Early returns or conditional layout

**Error Handling:**
- âœ… **ALWAYS**: useMuiSnackbar for user feedback
- âŒ **NEVER**: react-toastify
